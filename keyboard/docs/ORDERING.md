...
every surface-mount part for you — all 23 hotswap sockets, all 23 diodes, the
...
netlist, but no copper traces.** Open `build/pcbs/pinkyless46.kicad_pcb` in
...
1. **Open** `build/pcbs/pinkyless46.kicad_pcb` in KiCad 7, 8 or 9. It is saved
...
|----:|------|---------|-------|
| 23 | Kailh Choc v1 hotswap socket, **CPG135001S30** | SMD | JLC assembles. **Verify stock first — see below.** |
| 23 | **1N4148W** diode | SOD-123 | JLC assembles. Extremely common, always in stock. |
...
| 23 | Kailh Choc v1 switch | — | Your choice of weight. |
| 23 | Choc keycap (18 × 17 mm) | — | MBK, Chicago Steno, etc. |
...
2. Order the boards bare and hand-solder the sockets — they are 2 pads each,
   46 joints per half, tedious but not difficult.
